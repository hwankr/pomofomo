-- Debug script for A->B rejected, B->A fails
...
begin
  ...
  if existing_request is null then
    ...
    raise notice 'Request found: id=%, status=%', existing_request.id, existing_request.status;
    
    if existing_request.status = 'rejected' then
      raise notice 'Attempting update...';
      update friend_requests 
      set status = 'pending',
          created_at = now(),
      ...
      if found then
        ...
        raise notice 'Update failed (row not found?)';
      end if;
    else
      raise notice 'Status is not rejected';
    end if;
  end if;
end;
$$;
